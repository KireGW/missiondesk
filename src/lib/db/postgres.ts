...
`,
  ...
  {
    id: "0007_normalize_event_date_nulls_postgres.sql",
    sql: `
-- Cleanup-only migration: normalize known MVP placeholder strings.
-- Keep the column as TEXT for backward compatibility until Upcoming Signals
-- gets a dedicated event model.
UPDATE processed_items
SET event_date = NULL,
    updated_at = ${nowTextSql}
WHERE event_date IS NOT NULL
  AND (
    btrim(event_date) = ''
    OR lower(btrim(event_date)) IN ('null', 'undefined', 'nan')
  );
`,
  },
  {
    id: "0008_temporal_signals_postgres.sql",
    sql: `
